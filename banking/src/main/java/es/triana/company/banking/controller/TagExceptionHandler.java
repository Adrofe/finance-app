...
@RestControllerAdvice(assignableTypes = TagsController.class)
public class TagExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleTagValidationException(TagValidationException exception) {
        ApiResponse<Void> response = new ApiResponse<>(400, exception.getMessage(), null);
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
    }

    @ExceptionHandler(TagNotFoundException.class)
    public ResponseEntity<ApiResponse<Void>> handleTagNotFoundException(TagNotFoundException exception) {
    ...
    public ResponseEntity<ApiResponse<Void>> handleTagConflictException(TagConflictException exception) {
